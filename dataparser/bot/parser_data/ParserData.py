...
class ParserData:
    ...
    async def start_parse_event(self, data: dict):
        ...
        for target_url in task.urls.split("::"):
            ...
            task_item_data_with_id = add_task_item_in_db.delay(
                task_item.to_dict()).get()

            task_item_id = task_item_data_with_id['task_item_id']

            task_item_data_with_id['raw_data']['id'] = task_item_id
            task_item_data_with_id['raw_data']['uid'] = task.user_id

            await self.add_task_to_parsing_queue(
                task_item=task_item_data_with_id)
    ...
    async def add_task_to_parsing_queue(self, task_item: dict):
        ...
        await self.db.update_task_item(
            task_item_id=task_item['raw_data']['id'],
